...
const InternaitionaCSVCampaign = () => {
  ...
  // Local states for campaign details, media, editor and form values
  ...
  // New states for CSV column selection
  const [mobileColumn, setMobileColumn] = useState("a");
  const [mediaUrlColumn, setMediaUrlColumn] = useState("b");

  // API data for groups and message templates
  const [groups, setGroups] = useState([]);
  const [msgTemplates, setMsgTemplates] = useState([]);

  // File & preview storage for images, PDF, and video
  ...
  // Caption states for images, PDF and video
  const [imageCaptions, setImageCaptions] = useState(["", "", "", ""]);
  const [pdfCaption, setPdfCaption] = useState("");
  const [videoCaption, setVideoCaption] = useState("");

  // State for country dropdown (for dialing code)
  const [countries, setCountries] = useState([]);
  const [selectedCountry, setSelectedCountry] = useState("");

  // Alphabet array for column dropdowns
  const alphabet = Array.from({ length: 26 }, (_, i) =>
    String.fromCharCode(65 + i)
  );

  // Fetch groups and message templates from API on mount
  useEffect(() => {
    axios
  ...
  // Fetch countries for dialing code
  useEffect(() => {
    axios
  ...
  // When a group is selected, populate the WhatsApp numbers textarea
  useEffect(() => {
    if (selectedGroup !== "") {
  ...
  // Drag & drop & file input for Excel/CSV file
  ...
  // Process the CSV/Excel file by calling the uploadExcel API endpoint.
  // This extracts WhatsApp numbers and media URLs and then appends the media URLs into the Froala editor.
  const handleCsvProcessing = async () => {
    ...
    // NOTE: Use the key "uploadExcel" as expected by your API
    ...
    try {
      ...
      // Process media URLs and append them to the Froala editor content
  ...
  // Submit the campaign data (the CSV file is processed separately)
      ...
      <div className="w-full px-4 mt-8">
        <div className="w-full py-2 mb-3 bg-white">
          <h1
            ...
            style={{ fontSize: "32px" }}
          >
            Internaitiona Csv Campaign
          </h1>
        </div>

        <div className="flex gap-6">
          {/* Left Side: Campaign Title, Country, Group, and WhatsApp Numbers */}
          <div className="w-2/5 flex flex-col gap-6">
            ...
            {/* Country Dropdown for Mobile Code */}
          ...
          {/* Right Side: Templates, CSV Upload, Editor, and Media Upload */}
...
export default InternaitionaCSVCampaign;
